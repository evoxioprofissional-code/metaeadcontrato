-- Terceiro contrato: LICENCIATURA EM PEDAGOGIA (texto fiel enviado pela escola)
-- + Cláusula 21ª de REMATRÍCULA semestral (acrescentada a pedido do dono).
with c as (
  insert into public.contracts (title, description, active)
  values ('Contrato de Prestação de Serviços — Licenciatura em Pedagogia',
          'Contrato do curso de Licenciatura em Pedagogia.', true)
  returning id
)
insert into public.contract_versions (contract_id, version, content_html, is_published)
select c.id, '1.0', $html$
<h2>CONTRATO DE PRESTAÇÃO DO CURSO DE: LICENCIATURA EM PEDAGOGIA</h2>
...
<p><strong>Cláusula 1ª</strong> - O CONTRATANTE contrata à CONTRATADA para realização de CURSO DE LICENCIATURA EM PEDAGOGIA.</p>
...
<p><strong>Cláusula 17ª</strong> - O CONTRATANTE terá um período de 3 (três) meses após o termino do curso para sanar todas as pendências administrativa e/ou financeira, caso contrário será invalidado o período do tempo já cursado.</p>
...
<p><strong>Cláusula 20ª</strong> - No final do curso, o aluno pagará 01 taxa de CERTIFICAÇÃO, no valor de R$ 900,00, que poderá ser dividida em 03 vezes.</p>
<p><strong>Cláusula 21ª — REMATRÍCULA (SEMESTRAL)</strong> - A continuidade do curso depende de rematrícula a cada 6 (seis) meses. O valor da rematrícula corresponde a 01 (uma) mensalidade, podendo haver desconto conforme a promoção/condição vigente oferecida pela CONTRATADA à época. A não realização da rematrícula no prazo poderá acarretar a suspensão das aulas até a regularização.</p>
